...
var RecipesUI = Class.create(
{
    ...
    ********************************************************/
    ...
    SEARCH_PERPAGE:         9,      //Number of search results per page
    ...
    ********************************************************/
    ...
    initialize: function()
    {
        ...
        this.$searchInput       = $('input_recipes_search');
    ...
    ********************************************************/
    ...
    init: function()
    {
        ...
        if(!Kookiiz.panels.is_disabled('recipes'))
        {
            ...
            //Recipe form link
            $('recipeform_link').observe('click', this.form_open_click.bind(this));
        ...
        //SEARCH PANEL
        if(!Kookiiz.panels.is_disabled('search'))
        {
            ...
            $('recipes_search_button').observe('click', this.search_change.bind(this));
            $('select_search_category').observe('change', this.search_change.bind(this));
            $('select_search_origin').observe('change', this.search_change.bind(this));
            $$('input.check_search').invoke('observe', 'click', this.search_change.bind(this));
            $$('select.select_search').invoke('observe', 'change', this.search_select.bind(this));
            Utilities.observe_return(this.$searchInput, this.search_change.bind(this));
    ...
    ********************************************************/
    ...
    //#recipe_object (DOM): recipe DOM element
    //-> (void)
    preview_setup: function(element)
    {
        if(this.preview_enabled)
        {
            ...
            var recipe_position_abs     = element.cumulativeOffset();			//Recipe position from absolute top left corner of the website
            var recipe_position_scroll  = element.cumulativeScrollOffset();     //Amount by which recipe was scrolled
            var recipe_position_rel     = element.viewportOffset();             //Recipe position from top left corner of current view
            var recipe_dimensions       = element.getDimensions();				//Recipe box dimensions
            ...
            //If there is enough space on the left and on the right, center the preview on the recipe box
            //Else try to display it on the left or on the right
            var display_left = 0;
            if(recipe_space_left > (display_dimensions.width - recipe_dimensions.width) / 2
            && recipe_space_right > (display_dimensions.width - recipe_dimensions.width) / 2)
                display_left = viewport_scroll.left + recipe_position_abs.left - (display_dimensions.width - recipe_dimensions.width)/2;
            else if(recipe_space_left >= recipe_space_right)
    ...
    ********************************************************/
    ...
    //#summary (bool):      if true a search summary is displayed under the results (defaults to true)
    //-> (void)
    search: function(criteria, summary)
    {
        //Default values
        ...
        {
            this.themes_toggle('up', this.search.bind(this, this.criteria, summary));
            return;
        }
        ...
        //This is to avoid that a longer and older search erase
        ...
        if(summary)
        {
            var text = this.search_summary(this.criteria);
            this.search_summary_display(text);
        }
    },
    ...
    search_display: function()
    {
        ...
        this.elements_build(this.$searchResults, this.search_results, 'search',
        {
            'mode':         'box',
            'preview':      true,
            'sorting':      this.$searchSorting.value,
            'deletable':    true,
    ...
    ********************************************************/
    ...
    search_update: function()
    {
        ...
        var recipes = this.$searchResults.select('.recipe_box');
        if(!recipes.length) this.search_noresult();
    },
