...
var TabsUI = Class.create(Observable,
{
    ...
    ********************************************************/
    ...
    PANELS_SETS:    {
                        ...
                        'main':             ['facebook', 'feedback', 'fridge', 'recipes', 'shopping'],
                        'profile':          [],
    ...
    ********************************************************/
    ...
    {
        if(this.mode == 'state')
            window.History.pushState({}, null, URL_HASH_TABS[0]);
        else
    ...
    ********************************************************/
    ...
    display_section: function()
    {
        ...
        if(menu_show)
            Kookiiz.menu.slatesShow();
        else
            Kookiiz.menu.slatesHide();
    },
    ...
    ********************************************************/
    ...
    init: function()
    {
        this.mode = 'hash'; //Utilities.available.history ? 'state' : 'hash';
    ...
    ********************************************************/
    ...
    listen: function()
    {
        if(this.mode == 'state')
        {
            window.History.Adapter.bind(window, 'statechange', this.onStateChange.bind(this));
            this.onStateChange({'initial': true}, true);
        }
        else
    ...
    ********************************************************/
    ...
    onStateChange: function()
    {
        var state   = window.History.getState(),
            initial = state.data.initial || false,
            ignore  = state.data.ignore || false, 
            hash    = state.hash;
        if(ignore) return;
        if(initial && (hash == '/' || hash == '/#/'))
            this.defaultTab();
        else
        {
            //Remove leading "/" and split on "-"
            hash = hash.gsub(/\//, '').sub(/#/, '').split('-');
            //Check tab value
            var tab = hash[0],
                tab_id = URL_HASH_TABS.indexOf(tab),
                tab_name = TABS[tab_id];
    ...
    ********************************************************/
    ...
    show: function(tab_name, content_id, content_title)
    {
        ...
        else if(content_title != this.tempTitleGet())
        {
            ...
            if(this.mode == 'state')
                window.History.pushState({'ignore': true}, null, tab_url);
            else
