...
/*
 * Update by Kookiiz Team (13.11.2010):
 * changed name of global variable from Hash to URLHash for compatibility with Prototype "Hash" object
*/
...
/* 
 * Added by Kookiiz Team (17.03.2011)
*/
//Wrapper to trigger callback function and pass it a hash without the '/' prefix
//#initial (bool): whether the callback is triggered for the first time
//-> (void)
callbackTrigger = function(initial)
{
    var index = hash.indexOf('/');
    var clean_hash = index == -1 ? '' : hash.substr(index + 1);
    callback(clean_hash, initial);
},
...
poll = function () {
    ...
    if (curHash != hash) {
        hash = curHash;
        /* Update by Kookiiz Team (17.03.2011) */
...
return {
    ...
    //Added "ignore" parameter to change the hash without callback triggering
    go: function (newHash, ignore) {
        if(typeof(ignore) == 'undefined') ignore = false;
        /* 
         * Update by Kookiiz Team (17.03.2011):
         * added some formating for hashes to match the pattern: #/hash
         */
        newHash = '/' + newHash;
        ...
        if (iframe) {
            ...
            if(!ignore) callbackTrigger(false);
        }
    }
};
})();