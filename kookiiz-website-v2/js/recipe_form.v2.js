...
var RecipeForm = Class.create(
{
    ...
    ********************************************************/
    ...
    descriptionUpdate: function()
    {
        ...
        {
            element = steps[i].select('.text')[0];
            text    = element.innerHTML.stripTags();
            description += text;
            if(i < imax - 1)
                description += '\n\n';
        }
    ...
    ********************************************************/
    ...
    onStepEdit: function(event)
    {
        ...
        var step   = event.findElement('.step'),
            ...
            disp   = step.select('.text')[0],
            icon   = step.select('img.edit')[0];

        //Remove number at the beginning of the string
        var text = disp.innerHTML;
        if(text.search(/[0-9]+.\s/) === 0)
            disp.innerHTML = text.sub(/[0-9]+.\s/, '');
        input.swap(disp, this.onStepValidate.bind(this), icon);
    },
    ...
    onStepValidate: function(event)
    {
        ...
        input.swap(disp, this.onStepEdit.bind(this), icon);
        this.descriptionOrder();
    },
    ...
    onSubmitConfirm: function(response)
    {
        ...
        else if(action == 'save')
        {
            ...
            Kookiiz.popup.custom(
            {
                ...
                'confirm':      true,
                'cancel':       false,
                'content_url':  '/dom/recipe_submit_popup.php',
                'content_init': recipe['public'] ? this.onSubmitConfirmPopup.bind(this, recipe_id, recipe['name']) : false
            });
        }
    ...
    //Called once the recipe submission popup is ready
    //-> (void)
    onSubmitConfirmPopup: function(recipe_id, recipe_name)
    {
        var share_button = $('recipeform_submit_popup').select('p.share_button')[0];
        var like_url = 'http://www.kookiiz.com/' + URL_HASH_TABS[4] + '-' + recipe_id + '-' + Utilities.text2link(recipe_name),
            like_button = '<fb:like href="' + like_url + '" layout="button_count" font="tahoma" send="true"></fb:like>';
        share_button.innerHTML = like_button;
        FB.XFBML.parse(share_button);
    },
